...
//	header = 0x08c7;
	...
	switch (unit->group->skill_id)
	{
		case WZ_STORMGUST:
		{
			unit_id = (&unit->group->unit[unit->group->unit_count / 2] != unit) ? 0x20 : 0x10;
		}
		break;

		case WZ_VERMILION:
		{
			unit_id = (&unit->group->unit[unit->group->unit_count / 2] != unit) ? 0x22 : 0x12;
		}
		break;
	...
	switch (header) {
		...
		case 0x09ca:
			...
			WBUFL(buf,pos+21) = unit->group->tick;
			len += 4;
			WBUFW(buf,pos) = len;
			break;
	}
