...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PRONTERA,    156, 181 }, //  0=Prontera
		{ MAP_MORROC,      156,  93 }, //  1=Morroc
		{ MAP_GEFFEN,      50,  115 }, //  2=Geffen
		{ MAP_PAYON,       158, 84 }, //  3=Payon
		{ MAP_ALBERTA,     53, 53 }, //  4=Alberta
#ifdef RENEWAL
		{ MAP_IZLUDE,      128, 146 }, //  5=Izlude (Renewal)
#else
		{ MAP_IZLUDE,      128, 114 }, //  5=Izlude
#endif
		{ MAP_ALDEBARAN,   141, 104 }, //  6=Al de Baran
		{ MAP_LUTIE,       148, 126 }, //  7=Lutie
		{ MAP_COMODO,      209, 143 }, //  8=Comodo
		{ MAP_YUNO,        158,  118 }, //  9=Yuno
		{ MAP_AMATSU,      112,  139 }, // 10=Amatsu
		{ MAP_GONRYUN,     160, 113 }, // 11=Gonryun
		{ MAP_UMBALA,       138, 207 }, // 12=Umbala
		...
		{ MAP_JAWAII,      205, 285 }, // 17=Jawaii
		{ MAP_AYOTHAYA,    150, 111 }, // 18=Ayothaya
		{ MAP_EINBROCH,     64, 200 }, // 19=Einbroch
		{ MAP_LIGHTHALZEN, 158,  92 }, // 20=Lighthalzen
		{ MAP_EINBECH,      173,  121 }, // 21=Einbech
		{ MAP_HUGEL,        96, 126 }, // 22=Hugel
		{ MAP_RACHEL,      115, 129 }, // 23=Rachel
		{ MAP_VEINS,       216, 123 }, // 24=Veins
		{ MAP_MOSCOVIA,    223, 184 }, // 25=Moscovia
//		{ MAP_MIDCAMP,     180, 240 }, // 26=Midgard Camp
//		{ MAP_MANUK,       282, 138 }, // 27=Manuk
//		{ MAP_SPLENDIDE,   194, 138 }, // 28=Splendide
//		{ MAP_BRASILIS,    174, 184 }, // 29=Brasilis
//		{ MAP_DICASTES,    198, 222 }, // 30=El Dicastes
...
ACMD_FUNC(lgp)
{
	if (sd->lgp_settings & LGP_ENABLED)
	{
		...
		clif_displaymessage(sd->fd, "LGP disabled!");
	}
	else
	{
		sd->lgp_settings |= LGP_ENABLED;	
		clif_displaymessage(sd->fd,"LGP enabled!");
	}

	pc_setglobalreg(sd, add_str("LGP_SETTINGS"), sd->lgp_settings);
...
{
	int size;
	char * command_info = "Usage @square <on/off/1-14>";
	...
	else if(!strcmpi(message,"off"))
	{
		...
		clif_displaymessage(fd,"Square disabled!");
	}
	else if(!strcmpi(message,"on"))
	{
		sd->lgp_settings |= LGP_SQUARE;	
		clif_displaymessage(fd,"Square enabled!");
	}
	else if((size = atoi(message)) >= 1 && size <= 14)
	{
		sd->lgp_settings &= 0xFFFFFF00;
		sd->lgp_settings += size;
		clif_displaymessage(fd,"Square size setted!");
	}
	else
	{
		clif_displaymessage(fd, command_info);
...
ACMD_FUNC(circle)
{
	if (sd->lgp_settings & LGP_CIRCLE)
	{
		...
		clif_displaymessage(sd->fd, "Circle disabled!");
	}
	else
	{
		sd->lgp_settings |= LGP_CIRCLE;	
		clif_displaymessage(sd->fd, "Circle enabled!");
	}
...
ACMD_FUNC(aoes)
{
	if (sd->lgp_settings & LGP_AOES)
	{
		...
		clif_displaymessage(sd->fd, "Aoes disabled!");
	}
	else
	{
		sd->lgp_settings |= LGP_AOES;	
		clif_displaymessage(sd->fd, "Aoes enabled!");
	}
...
ACMD_FUNC(shake)
{
	if (sd->lgp_settings & LGP_SHAKE)
	{
		...
		clif_displaymessage(sd->fd, "Shake disabled!");
	}
	else
	{
		sd->lgp_settings |= LGP_SHAKE;	
		clif_displaymessage(sd->fd, "Shake enabled!");
	}
